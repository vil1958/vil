[## Project description](https://pypi.org/project/yt-dlp/)

Official repository: [https://github.com/yt-dlp/yt-dlp](https://github.com/yt-dlp/yt-dlp)

[##### EMBEDDING YT-DLP](https://github.com/yt-dlp/yt-dlp#:~:text=test%20an%20extractor.-,EMBEDDING%20YT%2DDLP,-yt%2Ddlp%20makes)
#python
...
##### [](https://github.com/yt-dlp/yt-dlp#embedding-examples)Embedding examples

#### [](https://github.com/yt-dlp/yt-dlp#extracting-information)Extracting information:
```
import json
import yt_dlp
...
http://forum.ru-board.com/topic.cgi?forum=5&topic=49719&start=1000#20

Чтоб послать линк с Youtube, soundcloud и т.п. сабжу из браузера я использую два расширения:    
[Youtube 2 Player](https://github.com/sumzary/yt2p/releases) удобно для пкм. Например отправить ролик в mpc-hc, просмотреть и сохранить если что прямо из него. Важно: из MPC-BE ролик не будет сохраняться с помощью *dl  
[External Application Button](https://add0n.com/external-application-button.html) для прямого скачивания текущего (открытого) ролика (тутор по установке смотреть обязательно!).  
yt-dlp лежит в отдельной папке вместе с ffmpeg и зареган в системе, чтоб любая программа без труда находила их самостоятельно: **( ! пути выдуманы)**    
**Код:**
```
Windows Registry Editor Version 5.00  
   
[HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\yt-dlp.exe]  
@="C:\\VideoTools\\yt-dlp.exe"  
"Path"="C:\\VideoTools"  
   
[HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\FFmpeg.exe]  
@="C:\\VideoTools\\ffmpeg.exe"  
"Path"="C:\\VideoTools"
```

Так же с пользой имею yt-dlp.conf    
**Код:**
```
--ffmpeg-location "C:\VideoTools\ffmpeg.exe"  
-f "bv+ba/b"  
--quiet  
--console-title  
-P temp:"%Temp%"  
...
Лаконично-симпатичный батник для скачки на раб.стол (предполагается что ffmpeg и yt-dlp лежат рядом в одной папке!):  
  **Код:**
```
@echo off  
...
set savedir=E:\Downloads\  
set file=%%(title)s  
yt-dlp.exe --console-title --quiet -P temp:"%Temp%" --ignore-config -f "bv+ba/b" "%url%" -o "%savedir%\%file%"
```
Собрал с коленки порташку [Yt-dlp Downloader.exe](http://file.sampo.ru/6tz56f/)    
Внутри ffmpeg.exe 6.0, yt-dlp 2023.03.04 и вышеописанный батник для ввода url. Всё это распаковывается в temp и после скачки самоудаляется. (чтобы обновить сборку напомните в л.с)

http://forum.ru-board.com/topic.cgi?forum=5&topic=49719&start=1373&limit=1&m=2#1
Для работы батника нужен NirCmd. NirCmd - утилита для выполнения некоторых задач, не отображая пользовательский интерфейс.  
[http://www.nirsoft.net/utils/nircmd.html](http://www.nirsoft.net/utils/nircmd.html)  
При первом запуске предлагает скопировать nircmd.exe в C:\Windows (для этого запуск утилиты должен быть с правами администратора). Папка C:\Windows по умолчанию прописана в Переменные среды в Path, поэтому дополнительные манипуляции не потребуются.
```
REM d:\T\bin\www\downloader\yt-dlp\yt-dlp.exe
REM d:\bin\batch\nircmd\x64\nircmd.exe
@echo off  
...
REM ::Чтение из буфера обмена скопированной ссылки с помощью утилиты nircmd  
for /f "delims=" %%i in ('nircmd.exe consolewrite ~$clipboard$') do set URL=%%i  
yt-dlp -f "bestvideo[height<=1080][dynamic_range=?SDR]+bestaudio/bestvideo[height<=1080][ext=mp4][dynamic_range=?SDR]+bestaudio[ext=m4a]/best" "%URL%" -o "%dir%\%file%"  

pause
```

**Варианты кнопки скачивания с помощью yt-dlp**  
[Вариант 1](http://forum.ru-board.com/topic.cgi?forum=5&topic=49719&start=1373&limit=1&m=1#1) [?] Используется утилита **paste**.  
[Вариант 2](http://forum.ru-board.com/topic.cgi?forum=5&topic=49719&start=1373&limit=1&m=2#1) [?] Используется утилита **nircmd**.  
[Вариант 3](http://forum.ru-board.com/topic.cgi?forum=5&topic=49719&start=1373&limit=1&m=3#1) [?] Используется **mshta** - Microsoft HTML Application Host (поставляется с операционной системой).  
[Вариант 4](http://forum.ru-board.com/topic.cgi?forum=5&topic=49719&start=1373&limit=1&m=4#1) [?] Используется **Windows PowerShell v5.1+**.  
[Вариант 5](http://forum.ru-board.com/topic.cgi?forum=5&topic=49719&start=1373&limit=1&m=5#1) [?] Используется **Windows PowerShell v5.0-**.  
...
[### yt-dlp_README.md](https://github.com/yt-dlp/yt-dlp/raw/master/README.md)
